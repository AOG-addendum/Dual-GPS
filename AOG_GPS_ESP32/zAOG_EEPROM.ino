...
void headingRollCalc() {
	double LatDec1 = 0, LatDec2 = 0, LonDec1 = 0, LonDec2 = 0, Height1 = 0, Height2 = 0;
	...
	//heading calc: good values: calc heading + roll
		...
		if (GPSSet.RollDevice == 1) {
				...
				((GPSSet.headingAngleCorrection > 250) && (GPSSet.headingAngleCorrection < 290)))
			{
				...
				double heightDiff = double((UBXPVT1[UBXIdx1].hMSL - UBXPVT2[UBXIdx2].hMSL)) / 10;//cm
				if (heightDiff < 0) {
					heightDiff *= -1;
					rollInvert = true;
				}
				roll = atan2(heightDiff, GPSSet.AntDist) / PI180;
				...
				if (GPSSet.debugmodeHeading) {
					...
					Serial.print(" Antenna height (cm): "); Serial.print(GPSSet.AntHeight, 1);
