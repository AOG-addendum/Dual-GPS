...
void process_Request()
{
    ...
    for (byte n = 0; n < server.args(); n++) {
        ...
        if (server.argName(n) == "AntHeight") {
            temDoub = server.arg(n).toDouble();
            if ((temDoub <= 600) && (temDoub >= 0)) { GPSSet.AntHeight = temDoub; }
        }
        ...
        if (server.argName(n) == "velocityPWM_Pin") {
            temLong = server.arg(n).toInt();
            if ((temLong >= 0) && (temLong <= 40)) {
                GPSSet.velocityPWM_Pin = byte(temLong);
            }
        }
        if (server.argName(n) == "velocityPWM_Multiplier") {
            temLong = server.arg(n).toInt();
            if ((temLong >= 0) && (temLong <= 255)) {
                GPSSet.velocityPWM_Multiplier = int(temLong);
            }
        }        
...
void make_HTML01() {
    ...
    strcat(HTML_String, "<tr>");
    strcat(HTML_String, "<td>Antenna height (cm)</td>");
    strcat(HTML_String, "<td><input type = \"number\" onchange=\"sendVal('/?AntHeight='+this.value)\" name = \"AntHeight\" min = \"0\" max = \"600\" step = \"1\" style= \"width:100px\" value = \"");// placeholder = \"");
    strcati(HTML_String, int(GPSSet.AntHeight));
    strcat(HTML_String, "\"></td>");
    strcat(HTML_String, "</tr>");
    ...
    strcat(HTML_String, "To check this, the antenna distance measured by GPS is compared with antenna distance from the setting above.<br>");
    ...
    strcat(HTML_String, "Heading calculation is done if GPS antenna distance is less than 120 and more than 83.<br>");
    ...
    strcat(HTML_String, "With some bases for RTK, the flags will sometimes fail, causing the position to jump.<br>");
    strcat(HTML_String, "In this case turn the UBX flag check OFF<br>");
    strcat(HTML_String, "<table>");
    set_colgroup(300, 250, 150, 0, 0);
   // strcat(HTML_String, "<tr> <td colspan=\"3\">&nbsp;</td> </tr>");
    //checkbox
    ...
    //---------------------------------------------------------------------------------------------  
    // Velocity PWM output
    strcat(HTML_String, "<h2>Velocity PWM output</h2>");
    strcat(HTML_String, "PWM output (on pin selected below) in HZ per MPH. Set to 0 to disable.<br>");
    strcat(HTML_String, "Useful for various rate controllers like Topcon, Raven, and etc.<br><br>");
    strcat(HTML_String, "<form>");
    strcat(HTML_String, "<table>");
    set_colgroup(300, 250, 150, 0, 0);

    strcat(HTML_String, "<tr>");
    strcat(HTML_String, "<td></td><td><input type = \"number\" onchange=\"sendVal('/?velocityPWM_Multiplier='+this.value)\" name = \"velocityPWM_Multiplier\" min = \"0\" max = \"255\" step = \"0.1\" style= \"width:100px\" value = \"");// placeholder = \"");

    strcatf(HTML_String, GPSSet.velocityPWM_Multiplier, 3, 1);
    strcat(HTML_String, "\"></td>");
    strcat(HTML_String, "<td><input type= \"button\" onclick= \"sendVal('/?Save=true')\" style= \"width:120px\" value=\"Save\"></button></td>");
    strcat(HTML_String, "</tr>");
    ...
    strcat(HTML_String, "<td><b>pin for Velocity PWM</b></td>");
    strcat(HTML_String, "</tr>");
    for (int i = 2; i < 34; i++) {
        ...
        strcat(HTML_String, "<td><input type = \"radio\" onclick=\"sendVal('/?velocityPWM_Pin=");
        strcati(HTML_String, i);
        strcat(HTML_String, "')\" name=\"Velocity PWM\" id=\"GPIOVelocity\" value=\"");
        strcati(HTML_String, i);
        strcat(HTML_String, "\"");
        if (GPSSet.velocityPWM_Pin == i) { strcat(HTML_String, " CHECKED"); }
        strcat(HTML_String, "><label for=\"JZ");
        strcati(HTML_String, i);
        strcat(HTML_String, "\">");
        strcati(HTML_String, i);
