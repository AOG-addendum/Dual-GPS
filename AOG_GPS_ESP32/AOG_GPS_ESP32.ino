//ESP32 program for UBLOX receivers to send NMEA to AgOpenGPS or other program
...
//change settings to your requirements. Afterwards you can change them via the webinterface x.x.x.79 (192.168.1.79)
//if connection to your network fails an accesspoint is opened: webinterface 192.168.1.1

//use serial monitor on USB port to get debug messages and IP for webinterface when ESP starts.
...
struct set {
    ...
    int velocityPWM_Multiplier = 0; // PWM (MPH * multiplier)

  //WiFi---------------------------------------------------------------------------------------------
#if HardwarePlatform == 0
  //tractors WiFi
    char ssid[24] = "AOG hotspot";           // WiFi network Client name
    char password[24] = "password";                 // WiFi network password//Accesspoint name and password

    char ssid_ap[24] = "Dual GPS";  // name of Access point, if no WiFi found, NO password!!
    int timeoutRouter = 90;                //time (seconds) to search for existing WiFi, than starting Accesspoint 

    byte timeoutWebIO = 10;                 //time (minutes) afterwards webinterface is switched off
    ...
    double AntHeight = 228.0;              //cm height of Antenna
    ...
    byte GPSPosCorrByRoll = 1;            // 0 = off, 1 = correction of position by roll (AntHeight must be > 0)
...
unsigned long NtripDataTime = 0, now = 0, WebIOTimeOut = 0, prev_PWM_Millis = 0;
bool WebIORunning = true;
bool velocityPWM_enabled = false;
...
void setup()
{
    ...
    delay(10);
    if ((GPSSet.velocityPWM_Multiplier != 0) && (GPSSet.velocityPWM_Pin != 0)) {
       pinMode( ( uint8_t ) GPSSet.velocityPWM_Pin, OUTPUT);
       ledcSetup( 0, ( uint8_t ) GPSSet.velocityPWM_Multiplier, 8 );
       ledcAttachPin( ( uint8_t ) GPSSet.velocityPWM_Pin, 0 );
       ledcWrite(0, 128);
       velocityPWM_enabled = true;  
     }
    Serial.print("PWM is ");
    Serial.print(GPSSet.velocityPWM_Multiplier);
    Serial.print("hz per MPH on pin ");
    Serial.println(GPSSet.velocityPWM_Pin);
#endif
...
void loop()
{
	...
	{//Serial.println("new UBX to process");
		...
		if (existsUBXRelPosNED) {
			...
			if ((GPSSet.virtAntForew != 0) || (GPSSet.virtAntRight != 0) ||
				((GPSSet.GPSPosCorrByRoll == 1) && (GPSSet.AntHeight > 0)))
			{//all data there
				virtualAntennaPoint();
			}
		}
    ...
    if (my_WiFi_Mode == 1) { // in Station mode, reconnect when connection fails
      while (WiFi.status() != WL_CONNECTED) {
        WiFi_Start_STA_Sync();
      }
    }
    else if (my_WiFi_Mode == 2) { // try connecting to the configured WiFi even if we create a soft AP
      WiFi_Start_STA_Async();
      if (WebIORunning == false and WiFi.status() == WL_CONNECTED) { // if no web requests, turn off AP, and switch to Station only      
  ...
  if (velocityPWM_enabled) {
    if (millis() - prev_PWM_Millis > 100){
      prev_PWM_Millis = millis();
      double mph_Pwm = float(UBXPVT1[UBXRingCount1].gSpeed) * 0.0022369363; // 1 gSpeed = 0.0036 km/h = 0.0022369363 mile/h 
      mph_Pwm *= GPSSet.velocityPWM_Multiplier;
